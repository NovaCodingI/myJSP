...
<!-- 
	...
	로그인 기능을 구현 하는데 사용됩니다.
		...
		서버는 어떻게 로그인 상태를 유지 시켜 줄 수 있을까요?
		...
		클라이언트에서 웹브라우저로 서버에 최초로 접근시
			...
			-> 세션아이디는 쿠키로 생성되어 응답객체의 쿠키에 담겨 클라이언트로 전송
				...
				여기 저장된 값이 세션아이디 입니다.
		(웹브라우저를 새로 열고 서버에 접근시 쿠키에 저장된 세션 ID값이 변경된것을 확인 할 수 있습니다.)
	
	세션의 유지시간
		유지시간을 설정 하지 않으면 기본 30분(1800초)이며,
		유지시간이 만료되기전 새로운 요청이 들어오면 유지시간이 연장 됩니다.
		
		클라이언트가 유지시간내 요청이 없는 경우 해당 세션은 서버에서 제거 됩니다.
	...
	유지시간 설정 방법
		web.xml : 유지시간을 분단위로 설정 할 수 있습니다.
					유지시간 설정 후 웹브라우저를 닫았다가 다시 열어야 확인이 가능!!!
